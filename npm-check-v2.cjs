...
function runCmd(cmd) {
  try {
    ...
    // Even on error, npm usually prints valid JSON to stdout
    if (e.stdout) {
...
console.log('Scanning dependency tree for affected packages...')

// Run npm ls once for all packages
const json = runCmd('npm ls --json --long')
if (!json) {
  console.error('Could not run npm ls')
  process.exit(1)
}
...
try {
  ...
  console.error('Could not parse npm output', err)
  process.exit(1)
}
...
function checkDependencies(deps = {}) {
  for (const [pkg, info] of Object.entries(deps)) {
    if (affectedPackages[pkg]) {
      if (affectedPackages[pkg].includes(info.version)) {
        report.found.push({ package: pkg, version: info.version })
        // console.error(`[ALERT] AFFECTED: ${pkg}@${info.version}`)
      } else {
        report.warn.push({ package: pkg, version: info.version })
        // console.warn(`[+] ${pkg} present but version ${info.version} not in affected list`)
      }
    } else {
      report.notFound.push({ package: pkg, version: info.version })
      // console.log(`[-] ${pkg} not in affected list`)
    }
...
// Walk full tree
checkDependencies(tree.dependencies)

if (report.found.length) {
  console.error('\x1b[31m%s\x1b[0m', '\n=== AFFECTED PACKAGES FOUND ===')
  for (const pkg of report.found) {
    console.error(`- ${pkg.package}@${pkg.version}`)
  }
}
if (report.warn.length) {
  console.log('\x1b[33m%s\x1b[0m', '\n=== PACKAGES FOUND, version not in affected ===')
  for (const pkg of report.warn) {
    console.warn(`- ${pkg.package}@${pkg.version}`)
  }
}
if (!report.found.length && !report.warn.length) {
